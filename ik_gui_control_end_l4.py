...
# Set ROS_PACKAGE_PATH to find meshes (needed for visual/collision meshes)
...
def ik_thread_func():
    ...
    # Setup Tasks
    try:
        left_task = solver.add_frame_task("openarm_left_link7_pico", np.eye(4)) # eye for I (identity),lmao
        ...
        # Elbow tasks (optional but good for stability)
        left_link4_task = solver.add_frame_task("openarm_left_link4_ext", np.eye(4))
        left_link4_task.configure("openarm_left_link4_ext", "soft", 1.0, 0.01)

        right_link4_task = solver.add_frame_task("openarm_right_link4", np.eye(4))
        right_link4_task.configure("openarm_right_link4", "soft", 1.0, 0.01)
    except Exception as e:
    ...
    with state.lock:
        ...
        # Initialize elbows
        left_l4_pose = robot.get_T_world_frame("openarm_left_link4_ext")
        right_l4_pose = robot.get_T_world_frame("openarm_right_link4_ext")

        # Elbow (link4) targets: offset defined in world frame (no rotation needed)
        # Goal: right arm target is back-right, left arm target is back-left.
        elbow_back = 0.15   # meters, along -X in world frame
        elbow_side = 0.15   # meters, +Y for right, -Y for left in world frame
        elbow_up = 0.10     # meters, along +Z in world frame

        # World frame offset vectors (directly add, no rotation transform needed)
        offset_l_world = np.array([-elbow_back, elbow_side, +elbow_up], dtype=np.float64)
        offset_r_world = np.array([-elbow_back, -elbow_side, +elbow_up], dtype=np.float64)

        # Directly add offset in world frame
        l4_pos_l = left_l4_pose[:3, 3].copy() + offset_l_world
        l4_pos_r = right_l4_pose[:3, 3].copy() + offset_r_world

        # Update elbow tasks once (assign full matrices to ensure the task target is actually updated)
        T_left_link4 = np.eye(4)
        T_left_link4[:3, 3] = l4_pos_l
        T_left_link4[:3, :3] = left_l4_pose[:3, :3] @ effector_rot_offset.T
        left_link4_task.T_world_frame = T_left_link4

        T_right_link4 = np.eye(4)
        T_right_link4[:3, 3] = l4_pos_r
        T_right_link4[:3, :3] = right_l4_pose[:3, :3] @ effector_rot_offset.T
        right_link4_task.T_world_frame = T_right_link4
    ...
    while RUNNING:
        ...
        # Elbow (link4) real pose and target pose frames
        robot_frame_viz(robot, "openarm_left_link4")
        robot_frame_viz(robot, "openarm_right_link4")
        frame_viz("target_left_link4", left_link4_task.T_world_frame)
        frame_viz("target_right_link4", right_link4_task.T_world_frame)

        robot_frame_viz(robot, "openarm_left_link4_ext")
        robot_frame_viz(robot, "openarm_right_link4_ext")
        frame_viz("target_left_link4_ext", left_link4_task.T_world_frame)
        frame_viz("target_right_link4_ext", right_link4_task.T_world_frame)
...
class ControlGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("OpenArm IK Control")
        ...
        ttk.Label(top_frame, text="按住按钮连续移动").grid(row=0, column=0, columnspan=3, sticky=tk.W)
