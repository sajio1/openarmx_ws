...
# OpenArmX 使用说明书 PDF 生成脚本（完美封面版）
# 解决封面分割问题：使用独立的封面页，确保不被目录分割
# 作者: 成都长数机器人有限公司
...
# 清理旧文件
echo "[1/5] 清理旧文件..."
rm -f "$COVER_FILE" "$CONTENT_FILE" "$OUTPUT_FILE" "$PDF_FILE"
...
# 3. 合并封面和正文
echo "[4/5] 合并封面和正文..."
cat "$COVER_FILE" "$CONTENT_FILE" > "$OUTPUT_FILE"
echo "✓ 最终文档合并完成 (共 $(wc -l < "$OUTPUT_FILE") 行)"
