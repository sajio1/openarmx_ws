...
# Simple CAN Interface Setup Script for Robstride Motors
# Usage: setup/configure_socketcan.sh <interface> [options]
...
usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0 can0                     # CAN 2.0 at 1Mbps for Robstride motors"
    echo "  $0 can0 -fd                # CAN FD at 1Mbps/5Mbps (if supported)"
    echo "  $0 can0 -b 500000          # CAN 2.0 at 500kbps"
    echo "  $0 can0 -fd -d 8000000     # CAN FD with 8Mbps data rate"
    echo ""
    echo "Note: Robstride motors typically use CAN 2.0 at 1Mbps"
}
...
echo "Configuring $CAN_IF for Robstride motors..."
...
echo "✓ $CAN_IF is active and ready for Robstride motors"